...
if active:
    ...
    def selectFileForSaving(prompt:str, defaultDirectory:str=defaultDirectory, defaultFileName:str="untitled", defaultExtension:str="", fileTypes=(("All Files", "*.*"),)):
        if not active:
            raise RuntimeError("Attempted to use GUI while not active")
        import tkinter.filedialog
        if defaultFileName.endswith(defaultExtension):
            defaultBaseName = defaultFileName
        else:
